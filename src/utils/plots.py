...
def plot_learning_curve(
    ...
    out_path: str = "results/tabular_learning_curve.png",
    window: int = 100,
    show: bool = False,
):
    ...
    plt.xlabel("Episode")
    plt.ylabel("Return")
    plt.title("Tabular Q-learning: Learning Curve (Return)")
    plt.legend()
    plt.tight_layout()
...
def plot_dqn_curve(
    ...
    plt.xlabel("Episode")
    plt.ylabel("Return")
    plt.title("DQN (single NN): Learning Curve (Return)")
    plt.legend()
    plt.tight_layout()
    plt.savefig(out_path, dpi=200)
    if show:
    ...
    if os.path.exists(losses_path):
        ...
        if len(losses) > 0:
            ...
            plt.xlabel("Episode")
            plt.ylabel("Loss")
            plt.title("DQN (single NN): Training Loss")
            plt.legend()
            plt.tight_layout()
